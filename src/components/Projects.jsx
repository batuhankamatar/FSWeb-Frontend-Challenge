...
function Projects() {
  ...
  return (
    <section className={`projects-container flex flex-col items-center w-full h-auto transition-colors duration-300 ${darkMode ? 'bg-[#2A2A2A]' : 'bg-white'}`}>
      <div className='projects-content flex flex-col items-center pt-[50px] w-full lg:pt-[82px] lg:pb-[129px] gap-[36px] lg:w-full lg:max-w-[1064px]'>
        <h3 className={`font-Inter font-[500] text-[36px] ${darkMode ? 'text-[#AEBCCF]' : 'text-[#0A0A14]'}`}>
        ...
        <div className='projects-info flex flex-col gap-[80px] w-full max-w-[450px] lg:flex-row lg:w-full lg:max-w-[1064px] lg:gap-[64px]'>
          {projectsContent.list.map((project, index) => (
            <div
              key={index}
              className={`project-card relative flex flex-col w-full max-w-[500px] h-[668px] pt-[46px] rounded-[12px] 
               ${darkMode ? 'bg-[#2D3235]' : project.bgColor}`}
            >
              <div className="pl-[40px]">
                <h4 className={`font-['Playfair_Display'] font-[700] text-[29px] ${darkMode ? 'text-[#AEBCCF]' : 'text-[#0A0A14]'}`}>
                  {project.title}
                </h4>
                ...
                <div className='project-tags flex flex-wrap w-full max-w-[270px] h-auto pt-[27px] gap-[9px]'>
                  {project.technologies.map((tech, techIndex) => (
                    <span
                      key={techIndex}
                      className={`tag px-4 py-2 rounded-full bg-white font-['Playfair_Display'] font-[700] text-[16px] text-black`}
                    >
                      {tech}
                    </span>
                  ))}
